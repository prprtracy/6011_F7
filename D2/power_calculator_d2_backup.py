"""Preserved Delivery 2 implementation before the D3 feedback changes.

This file is an archival copy. Run ``power_calculator.py`` for the current
application.
"""
...
def exponential(value: float) -> float:
    ...
    while index <= MAX_SERIES_ITERATIONS:
        ...
        total += term
        if absolute_value(term) <= SERIES_TOLERANCE * absolute_value(total):
            if scale > 1023:
                raise NumericRangeError(
                    "The result is too large to be represented by this calculator."
                )
...
class PowerCalculatorApp:
    ...
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.status_label.grid(
            row=6, column=1, sticky="w", padx=(12, 0), pady=(12, 0)
        )
